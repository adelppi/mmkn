...
<!-- 0003 がスタック全体の親。0004〜0016 は 0003 を前提とする個別決定。0008 は 0004 の内側を、0009 は 0008 を前提に Web の画面を埋める。0010 は 0004 の「テスト」を、0011 は 0004・0005・0006・0010 が個別に要求している CI・リリースの手順を埋める。0012 は 0003 の「認証」の中身を（0007 を廃止して置き換え）、0016 は domain/money.md が要求するデータの出どころを（0013 を廃止して置き換え）、0014・0015 は運用でしか現れない穴（障害の気づき方・データの失われ方）を埋める。0001 はスタックに依存しない決定。廃止した ADR は削除せず、廃止の印とともに残す。 -->
...
| ログイン手段の規則（1 つ以上・1 サービスにつき 1 つ・最後の 1 つは削除できない）、追加・削除の振る舞い | `domain/group.md` |
| mmkn のアカウントを持たない人が外部サービスから操作したときに起きること | `domain/group.md` |
| ログイン手段を 1 つ失ったとき／すべて失ったときに起きること（復旧手段を持たないこと） | `domain/group.md` |
...
| 各層の内部構造、Controller と Presenter の役割、ユースケースの入出力と失敗の表し方、永続化の単位、識別子の生成、合成ルート、層ごとの import 規則、**セッションの読み取りとどのリクエストで更新するか**、**セッションの検証のしかた** | `adr/0008-layer-internals.md` |
| Web の画面構成（Container/Presentational）、ビューモデルの制約、UI ライブラリ、フォームの実装方式、クライアント側の入力検査の範囲、**上端の共有と待ちの切り離し**、**トーストの置き場**、**リクエストをまたぐキャッシュを持たない決定とその範囲**、**直前に見たものの再利用と先読み**、**取得を束ねる位置と、束ねてよい経路**、**画面設計の所在** | `adr/0009-web-ui.md` |
