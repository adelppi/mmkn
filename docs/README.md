...
<!-- 0003 がスタック全体の親。0004〜0011 は 0003 を前提とする個別決定。0008 は 0004 の内側を、0009 は 0008 を前提に Web の画面を埋める。0010 は 0004 の「テスト」を、0011 は 0004・0005・0006・0010 が個別に要求している CI・リリースの手順を埋める。0001 はスタックに依存しない決定。 -->
...
`glossary.md` と `operations.md` はこの 3 層の外側にある補助文書。前者は日本語とコードの名前を繋ぐ索引、後者は決定を実行に移す手順で、**どちらも一次情報を持たない**（`RULES.md` §9・§10）。
...
| 手元の準備・リリースの実行と失敗時の確認・診断の手順、実装着手時に確かめること | `operations.md` |
| まだ決まっていないこと | `open-questions.md` |
