...
mmkn が保存するお金の動きの記録を定義する。記録は **Payment（支払い）と Transfer（送金）の 2 種類だけ**。これらの記録から収支と清算案を導出する（`domain/settlement.md`）。
...
| 登録日時 | この Payment が登録された時点（後述「記録の並び」） |

- **内容は任意。空でもよい** [確定]。収支の計算に影響しないため、入力の手を止めさせない側を取る。空のときに一覧で何と表示するかは画面側の問題として扱う
- 内容は **100 文字以内**。前後の空白は落とす [確定]
...
| 登録日時 | この Transfer が登録された時点（後述「記録の並び」） |
...
- **未来の日付を指定できる** [確定]。前払いのように、これから起きる支払いを先に記録したい場面が実際にある。未来の日付であっても収支の計算は変わらず、日付が来るのを待って反映されるということもない
- 発生日は収支・清算案の計算に使わない [確定]。使うのは並び順と表示だけ

## 記録の並び [確定]

記録は **登録日時**を持つ。これは記録が登録された時点であり、**発生日とは別のもの**。同じグループの 2 つの記録について、どちらが先に登録されたかは必ず決まる。

一覧に並べるときは、**発生日の新しい順**に並べる。発生日が同じ記録どうしは、**登録日時の新しい順**（＝後から登録したものを先）に置く。

- 登録日時は日付ではなく**時点**であり、時刻を持つ。この点で発生日（時刻を持たない）とは扱いが異なる
- 登録日時は**編集で変わらない** [確定]。記録を直しても、一覧での位置は動かない。「最後に直したもの」ではなく「いつ登録されたか」の順序を保つため
- 登録日時が同じ記録が生じた場合の順序は、**記録の変わらない同一性による決定的な順序**で決める。並びが揺れることはない
...
- 起きないこと：
  ...
  - 編集前の内容は残らない
  - 収支・清算案をその場で書き換えることはしない（次に導出するときに、新しい記録から計算される）
...
収支も清算案も保存していないため、記録が変われば結論は自動的に追随する。詳細は `domain/settlement.md` を正とする。
...
| 登録日時 | 記録が登録された時点。並び順にだけ使い、編集で変わらない（上記「記録の並び」） |
| 登録者 | 記録を登録した User。何にも使わない |

どの記録が誰の収支をどちら向きに動かすかは `domain/settlement.md` を正とする。ここには再掲しない。
...
- 発生日が同じ記録が何件あっても、登録日時によって並びは一意に決まる [確定]
- 各操作の前提条件を満たさなかったときの扱いは `domain/group.md`「前提条件を満たさなかったとき」を正とする [確定]。**他のグループの記録を指した場合は「見つからない」として扱う**

## 他機能との関係

- `domain/group.md` … 支払者・負担者・送り手・受け手はすべて、同じグループの Member。前提条件を満たさなかったときの扱いもここが正
- `domain/money.md` … 金額と通貨の表し方
- `domain/settlement.md` … 記録から収支・清算案を導出する
