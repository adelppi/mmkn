...
| ログイン識別子 | ログインするときに、その人を指すもの。**mmkn 全体で重複しない** |
...
- ログイン識別子は、名前とは別のもの。名前を変えても、ログインの経路は変わらない [確定]。具体的に何を識別子とし、本人であることをどう確かめるかは `adr/0012-login.md` を正とする
...
mmkn は複数の提供形態を持ち、そのうちいくつかは外部サービスの上で使う（`overview.md`）。外部サービスのアカウントは、**2 つの異なる役割**で現れる。**この 2 つを混同すると、解除できるかどうかを取り違える。**

| 役割 | 何に使うか | 解除 |
|---|---|---|
| **ログインに使うもの** | 本人であることを確かめる。ログイン識別子はこのアカウントを指す | **できない** |
| **連携するもの** | その提供形態から操作されたときに「どの User の操作か」を解決する | できる |

どの外部サービスをどちらの役割に充てるかは `adr/0012-login.md`（ログイン）と `adr/0007-external-account-linking.md`（連携）を正とする。

#### ログインに使う外部アカウント

- **User は、ログインに使う外部アカウントを 1 つ持つ** [確定]。ログイン識別子は、そのアカウントを指すもの（前述「User の属性」）
- **解除する手段も、別のアカウントに移す手段も持たない** [確定]。解除できると、その User に到達する経路が無くなる
- **このアカウントを失うと、その User には誰も到達できない** [確定]。mmkn は復旧手段を持たない（後述「境界・例外ケース」）
- **これは連携ではない。** 連携している外部アカウントの一覧には現れず、下記の連携の規則も適用されない [確定]
...
- 前提条件：なし
- 入力：名前 / ログインに使う外部アカウント（本人であることが確かめられた状態で受け取る）
- 起きること：
  - User を 1 つ作成する
  - 名前は入力された名前になる
  - ログイン識別子は、そのアカウントを指すものになる
- 起きないこと：
  - **同じログイン識別子の User が 2 つできることはない。** 既に使われている識別子での作成は失敗する
  - Group・Member は作られない
  - **連携する外部アカウントは増えない。** ログインに使うものと連携するものは別の役割であり、ログインしたことが連携を意味することはない（前述「User と外部アカウント」）

**名前の入力欄の初期値に、外部サービス側の名前を使うかは未決** [保留]（`open-questions.md` #16）。名前が必須であること・20 文字以内であることは、どちらに決まっても変わらない（前述「User の属性」）。

### ログインする [確定]

- 前提条件：示された識別子の User が存在し、本人であることが確かめられること
...
| ログイン識別子 | ログインするときに User を指すもの | ログインの入口。名前とは別のもの |
...
- ログイン識別子が既に使われている状態でアカウントを作成しようとした場合、User は作られない [確定]
- **ログインに使う外部アカウントを失った場合、その User に到達する手段は無い** [確定]。復旧の経路を持たない。Member と過去の記録はそのまま残るが、その User として操作できる人がいなくなる。**同じ人が新しくアカウントを作っても、それは別の User** であり、以前の Member を引き継がない
...
- ログイン識別子を何にし、本人であることをどう確かめるかは `adr/0012-login.md` を正とする。ドメインとして要求するのは「mmkn 全体で重複しないこと」「本人であることが確かめられること」「1 User につき 1 つで、解除も移行もできないこと」までで、どの外部サービスに委ねるかは規定しない。[確定]
