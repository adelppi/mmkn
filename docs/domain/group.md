...
- 起きないこと：
  ...
  - その外部アカウントが既に別の User に連携されている場合、連携先は移らない [提案]
  - User の名前は、外部サービス側の名前で上書きされない [提案]
...
- 起きないこと：
  ...
  - その User が対応づけた「場と Group の対応」は変わらない [提案]
...
- 2 人が同時に同じグループに参加した場合、どちらも Member になる [提案]。場と Group の対応づけとは異なり、後から届いた方が失敗することはない
...
- 同じ場に対して、2 人が同時に別の Group を対応づけようとした場合、後から届いた方が失敗する [提案]（`domain/record.md`「同じ記録に同時に手が入ったとき」と同じ考え方）
