...
**更新するものが無いリクエストでは、認証基盤に問い合わせない。** 素通しするのは次の 3 つで、いずれも理由は「保存すべき新しいトークンが生まれない」ことである。
...
**画面の先読みは素通ししない。見分けられないためである** [確定]。中身まで先に取りにいく往復（`adr/0009`「直前に見たものを取り直さない」）は、**実際の遷移とまったく同じ形で届く。** 入口が見分けるための印は付かない。

見分けられないことを受け入れられるのは、**素通しの理由がここでは当たらない**ためである。先読みは利用者のブラウザ自身が出す往復であり、**返した cookie はそのまま手元に残る。** 期限内のトークンでは問い合わせが起きない（更新するものが無いため）ので、**平時の負荷も増えない。**

| 残る代償 | |
|---|---|
| 期限が切れた瞬間 | 遷移と先読みで**同じセッションの更新が同時に走り得る**。認証基盤が同じトークンでの更新を一定時間まとめて扱うことに乗っている |

**外れたときの現れ方は、下記の前提が外れたときと同じである。**
...
- **先読みを、入口が見分けられる形に留める**：素通しの表を変えずに済む。ただし見分けられる先読みは**形しか取ってこない**ため、`adr/0009`「直前に見たものを取り直さない」が狙った「押した先が既にある」状態にならない。
- **更新を、cookie を確実に書ける経路（文書の遷移・Server Action・Route Handler）だけで起こす**：先読みかどうかを見分けずに済む。ただし**画面の移動だけで過ごす間は更新の機会が無くなり**、この節がまさに避けようとした「しばらく使っていると勝手にログアウトされる」に近づく。
