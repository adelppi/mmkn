...
#### この節が対象とするのはサーバー側で共有されるキャッシュである [確定]

**ブラウザの中だけに存在し、他の利用者と共有される経路を持たないものは対象外とする。**

上の理由は「人ごとに鍵を分ける」ことから来ている。**鍵を分ける必要が無いものには当たらない。** 利用者の手元にあるものは、そもそも他の人のものが入り得ない。

#### 直前に見たものを取り直さない [確定]

**画面を移動したとき、取り直す範囲を、直前に見たものの再利用と先読みでさらに狭める。**

- 再利用の窓は **30 秒**とする
- **記録・収支・清算案の 3 つは、見ているタブから他の 2 つを先に取りにいく**（切り替えは 3 つの間でしか起こらないため、取りにいく先が確定している）

**記録を変える操作と、ログアウトは、手元にあるものをすべて捨てさせる。** そのため古くなり得るのは「他の人が別の端末で加えた変更」だけで、自分の操作の結果が古いまま見えることも、共有の端末に前の人の画面が残ることもない。

**古い表示から誤った書き込みは成立しない。** 書き込みは表示された値を信じず、その時点で確かめ直すためである（`domain/settlement.md`「清算案の送金を記録する」・`domain/record.md`「同じ記録に同時に手が入ったとき」を正とする）。**この決定は既にある性質に乗っているだけで、新しい保証を足していない。**

**先読みは実際の遷移と見分けられない。** 入口がそれをどう扱うかは `adr/0008`「どのリクエストで更新するか」を正とする。
...
- 良い点：
  ...
  - **タブの切り替えで待たない。** 先に取れていれば、押した時点で中身がある
- 留意点：
  - **Container を分けた分だけ同じ取得が重複する。** 束ねる仕組みを入れ忘れると、記録の全件取得が画面ごとに複数回走る
  - **束ねる範囲を書き込みの経路へ広げると、同時に手が入ったことの検出が黙って成立しなくなる。** 失敗しないため表に出ない
  - **他の人が別の端末で加えた変更は、再利用の窓のぶんだけ遅れて見える。** 自分の操作の結果は遅れない
  - **見られないタブの取得も走る。** 先読みするぶん、1 つの画面あたりのサーバー側の取得が増える
...
- **ブラウザ側でも一切取り直さない形を保つ**：表示が常にその時点のものになる。ただし**切り替えのたびに往復が要り、同じタブに戻るだけでも同じだけ待つ。** 手元にあるものは他の人と共有されないため、上の理由が当たらない。
- **先読みを持たず、再利用だけにする**：見られないタブの取得が走らない。ただし**初めての切り替えは待つ**ため、狙っていた「押した先が既にある」状態にならない。
- **再利用の窓をもっと長く取る**：切り替えがほぼ常に即座になる。ただし他の人が加えた変更の見えない時間が同じだけ伸びる。**手元の操作で消える以上、窓を伸ばして得られるのは「久しぶりに戻ったとき」だけである。**
